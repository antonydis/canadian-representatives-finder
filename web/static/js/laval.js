...
async function loadTranslations() {
    try {
        ...
        /* keep empty, t() falls back to key */
    }
}
...
function applyLang(lang, save = true) {
    ...
    if (
        currentPostal &&
        !document.getElementById("laval-results").classList.contains("hidden")
    ) {
        ...
        if (label)
            label.textContent =
                currentPostal.slice(0, 3) + " " + currentPostal.slice(3);
    }
}
...
const MEETING_LOCATION =
    "Hôtel de Ville temporaire, 3131 boul. Saint-Martin Ouest, Laval";
const MEETING_STREAM =
    "https://www.laval.ca/en/democratic-life/town-hall-elected-officials/city-council/seances-conseil-municipal/";
...
const DECISIONS = [
    ...
    // Mai 2026
    {
        title: "Emprunt de 334 M$ pour la mise à niveau de la station de récupération La Pinière",
        date: "2026-05-05",
        status: "approved",
        url: "https://vdldocgreffecmspc01sa.blob.core.windows.net/cms/SD-2026-1594_1.0.pdf",
    },
    {
        title: "Approbation en principe du projet Village Vert, un immeuble de cohabitat abordable",
        date: "2026-05-05",
        status: "approved",
        url: "https://vdldocgreffecmspc01sa.blob.core.windows.net/cms/SD-2026-1945_1.0.pdf",
    },
    {
        title: "Construction de la bibliothèque Alain-Grandbois, contrat adjugé à Afcor Construction",
        date: "2026-05-05",
        status: "approved",
        url: "https://vdldocgreffecmspc01sa.blob.core.windows.net/cms/SD-2026-1782_1.0.pdf",
    },
];
...
async function doSearch(raw) {
    ...
    try {
        ...
        if (data.success) {
            councillor = data.representatives.find(
                (r) =>
                    r.level === "municipal" &&
...
function renderMeeting() {
    ...
    today.setHours(0, 0, 0, 0);
    const next =
        MEETINGS.find((m) => parseLocalDate(m.date) >= today) ||
        MEETINGS[MEETINGS.length - 1];
    const rest = MEETINGS.filter(
        (m) => m !== next && parseLocalDate(m.date) >= today,
    ).slice(0, 5);
    ...
    const countdownClass =
        diff === 0 ? "meeting-countdown today" : "meeting-countdown";

    const datesChips = rest
        .map((m) => `<span class="date-chip">${formatShort(m.date)}</span>`)
        .join("");
...
function renderDecisions() {
    const rows = DECISIONS.map((d) => {
        const titleEl = d.url
            ? `<a class="dr-title" href="${esc(d.url)}" target="_blank" rel="noopener noreferrer">${esc(d.title)}</a>`
...
subForm.addEventListener("submit", async (e) => {
    ...
    const postal = document
        .getElementById("sub-postal")
        .value.trim()
        .toUpperCase()
        .replace(/\s/g, "");
    const phone = document
        .getElementById("sub-phone")
        .value.trim()
        .replace(/[\s\-\(\)\.]/g, "");
...
/* ── Correction modal ───────────────────────────────────────── */
document
    .getElementById("laval-correction-btn")
...
if (corrModal) {
    corrModal
        .querySelector(".laval-modal-close")
        ?.addEventListener("click", () => {
    ...
    corrModal
        .querySelector("#laval-corr-form")
        ?.addEventListener("submit", async (e) => {
            e.preventDefault();
            const msg = corrModal
                .querySelector("#laval-corr-message")
                .value.trim();
            const email = corrModal
                .querySelector("#laval-corr-email")
                .value.trim();
...
function closeSmsPopup() {
    ...
    document
        .getElementById("laval-sms-popup-backdrop")
        ?.classList.add("hidden");
}

document
    .getElementById("laval-sms-popup-close")
    ?.addEventListener("click", closeSmsPopup);
document
    .getElementById("laval-sms-popup-later")
    ?.addEventListener("click", closeSmsPopup);
document
    .getElementById("laval-sms-popup-backdrop")
    ?.addEventListener("click", closeSmsPopup);
...
document
    .getElementById("popup-subscribe-form")
    ?.addEventListener("submit", async (e) => {
        e.preventDefault();
        ...
        const postal = document
            .getElementById("pop-postal")
            .value.trim()
            .toUpperCase()
            .replace(/\s/g, "");
        const phone = document
            .getElementById("pop-phone")
            .value.trim()
            .replace(/[\s\-\(\)\.]/g, "");
        const consent = document.getElementById("pop-consent").checked;

        if (!name || name.length < 2) {
            errEl.textContent = t("laval_err_name");
            errEl.classList.remove("hidden");
            return;
        }
        if (!/^[A-Z]\d[A-Z]\d[A-Z]\d$/.test(postal)) {
            errEl.textContent = t("laval_err_postal");
            errEl.classList.remove("hidden");
            return;
        }
        if (!/^\+?1?\d{10,11}$/.test(phone)) {
            errEl.textContent = t("laval_err_phone");
            errEl.classList.remove("hidden");
            return;
        }
        if (!consent) {
            errEl.textContent = t("laval_err_consent");
            errEl.classList.remove("hidden");
            return;
        }
        ...
        try {
            ...
            errEl.textContent = t("laval_err_generic");
            errEl.classList.remove("hidden");
        } finally {
            btn.disabled = false;
            btn.textContent = t("laval_btn_subscribe");
...
(async () => {
    ...
    currentLang = LANGS.includes(saved)
        ? saved
